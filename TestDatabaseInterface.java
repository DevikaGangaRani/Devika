...
class cTestCrudsInterface 
{
    ...
    public static void main(String[] args) 
    {
	    try 
	    {
	        ...
	        if (className != null && dbConfig.length == 2) 
	        {
	            String dbName = dbConfig[0];
	            String tableName = dbConfig[1];
	            cUI oUI = new cUI(tableName, fieldNames);
	            iCRUDS oCruds = (iCRUDS) Class.forName(className).getConstructor(String.class, String[].class).newInstance(dbName, fieldNames);
	            while (true) 
	            {
	                System.out.println("\n======= Menu =======");
	                ...
	                {
	                    case 1:
	                        String[] recordDetails = oUI.getRecordAndFields();
	                        int rowInserted = oCruds.insertRecord(recordDetails, tableName);
	                        oUI.printStatusMessage(rowInserted, "insertion");
	                        break;
	                    case 2:
	                        String[][] records = oCruds.loadRecords(tableName);
